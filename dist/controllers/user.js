...
const getUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { user } = req;
        const existingUser = yield user_1.default.findById(user.id).select('-isAssistant -signinMethod').lean();
        if (existingUser) {
            const existingService = yield service_1.default.findOne({ assistant: user.id });
            if (existingService) {
...
const updateUser = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const user = yield user_1.default.findById(req.params.userId).select('-isAssistant -signinMethod').lean();
        if (user) {
            const existingService = yield service_1.default.findOne({ assistant: user.id });
            if (existingService) {
